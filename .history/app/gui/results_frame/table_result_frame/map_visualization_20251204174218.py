...
Status: Generating map data...
            ...
            # Add center marker with simplified popup (will be enhanced by JavaScript)
            folium.Marker(
                location=[lat, lon],
                popup=folium.Popup("<div id='marker-popup'>Click marker for data</div>", max_width=350),
                tooltip="Data Point - Click for details",
    ...
    def create_interactive_controls_html(self, data_dict, lat, lon, scale):
        ...
        <script>
            ...
            // Initialize - wait for Folium map to be ready
            window.addEventListener('load', function() {{
                ...
                if (typeof L !== 'undefined' && typeof L.heatLayer !== 'undefined') {{
                    ...
                    // Wait a moment for Folium to fully initialize the map
                    setTimeout(function() {{
                        // Find the Leaflet map object created by Folium
                        // The map variable name is in the HTML, let's search more carefully
                        const mapElements = document.querySelectorAll('.folium-map');
                        if (mapElements.length > 0) {{
                            const mapId = mapElements[0].id;
                            console.log('Map element ID:', mapId);
                        }}
                        
                        // Folium typically creates map with format: map_HASH
                        for (let key in window) {{
                            if (key.startsWith('map_') && window[key] instanceof L.Map) {{
                                leafletMap = window[key];
                                console.log('✓ Found Leaflet map object:', key);
                                console.log('Map object type:', typeof leafletMap);
                                console.log('Is L.Map instance:', leafletMap instanceof L.Map);
                                break;
                            }}
                        }}
                        ...
                        if (!leafletMap) {{
                            ...
                            alert('Error: Could not initialize map. Please refresh the page.');
                            return;
                        }}
