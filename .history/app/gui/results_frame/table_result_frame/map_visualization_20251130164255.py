...
class MapVisualizationWindow(ctk.CTkToplevel):
    ...
    def __init__(self, parent, gee_metadata: Optional[Dict] = None, data: Optional[pd.DataFrame] = None):
        ...
        # Check if data is time-series (has 'date' column with multiple rows)
        if self.data is not None and not self.data.empty:
            # Check for 'date' column (case-insensitive)
            date_columns = [col for col in self.data.columns if col.lower() == 'date']
            self.is_time_series = len(date_columns) > 0 and len(self.data) > 1
            
            # Debug print
            print(f"DEBUG: Columns in data: {list(self.data.columns)}")
            print(f"DEBUG: Number of rows: {len(self.data)}")
            print(f"DEBUG: Is time series: {self.is_time_series}")
