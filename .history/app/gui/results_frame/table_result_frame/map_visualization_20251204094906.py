...
class EmbeddedMapFrame(ctk.CTkFrame):
    """
    Embedded HTML map viewer using tkinter's Text widget with HTML rendering.
    Fallback to external browser if embedding fails.
    """
    ...
    def __init__(self, master, **kwargs):
        ...
        try:
            ...
            self.html_viewer = HtmlFrame(self, messages_enabled=False)
            self.html_viewer.pack(fill="both", expand=True)
            self.can_embed = True
        except ImportError:
            # Fallback: Show message and button
            self.setup_fallback_view()
    
    def setup_fallback_view(self):
        """Setup fallback view when embedding is not available."""
        self.info_label = ctk.CTkLabel(
            self,
            text="📍 Map View\n\nHTML embedding not available.\nInstall 'tkinterweb' for embedded view:\npip install tkinterweb\n\nClick button below to open in browser:",
            font=("Arial", 12),
            justify="center"
        )
    ...
    def load_html(self, html_file: str):
        """Load HTML file into viewer."""
        self.html_file = html_file
        
        if self.can_embed:
            try:
                self.html_viewer.load_file(html_file)
            except Exception as e:
                print(f"Error loading HTML: {e}")
                messagebox.showerror("Embedding Error", f"Failed to embed map: {str(e)}\nOpening in browser instead.")
