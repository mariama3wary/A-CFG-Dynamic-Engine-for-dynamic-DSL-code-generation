...
class EmbeddedMapFrame(ctk.CTkFrame):
    """
    Embedded map viewer using tkinterweb.
    Simple and reliable HTML rendering in tkinter.
    """
    ...
    def setup_webview(self):
        """Setup embedded web view using tkinterweb."""
        try:
            ...
            # Hide loading label after a moment
            self.after(500, lambda: self.loading_label.place_forget())
            
            # Block all event propagation from HtmlFrame to prevent CTk conflicts
            self.webview.bind("<Button-1>", lambda e: None)
            self.webview.bind("<Button-2>", lambda e: None)
            self.webview.bind("<Button-3>", lambda e: None)
            
            # Prevent the problematic mouse wheel binding
            def block_mousewheel(event):
                return "break"
            
            self.webview.bind("<MouseWheel>", block_mousewheel)
            self.webview.bind("<Button-4>", block_mousewheel)
            self.webview.bind("<Button-5>", block_mousewheel)
            
            # Stop event propagation to parent
            self.bind("<MouseWheel>", lambda e: "break")
            
            print("✓ tkinterweb loaded successfully")
            
        except ImportError:
            print("⚠ tkinterweb not installed - showing fallback view")
            self.setup_fallback_view()
        except Exception as e:
            print(f"⚠ Error loading tkinterweb: {e}")
            self.setup_fallback_view()
    
    def setup_fallback_view(self):
        """Show fallback view when tkinterweb is not available."""
        ...
        ctk.CTkLabel(
            info_frame,
            text="Map Embedding Unavailable",
            font=("Arial", 16, "bold")
        ).pack(pady=10)
        
        ctk.CTkLabel(
            info_frame,
            text="To view maps embedded in the window:\n\npip install tkinterweb\n\nThen restart the application",
            font=("Arial", 12),
            justify="center"
        ).pack(pady=20)
        ...
        ctk.CTkLabel(
            info_frame,
            text="Meanwhile, use the browser:",
            font=("Arial", 11),
    ...
    def load_html(self, html_file: str):
        ...
        if self.webview is not None:
            try:
                print(f"Loading map: {html_file}")
                
                # Show loading indicator
                if hasattr(self, 'loading_label'):
                    self.loading_label.configure(text="🔄 Loading map...")
                    self.loading_label.place(relx=0.5, rely=0.5, anchor="center")
                
                # Load the file
                self.webview.load_file(html_file)
                
                # Force update and hide loading label
                self.update_idletasks()
                self.after(1000, lambda: self.loading_label.place_forget() if hasattr(self, 'loading_label') else None)
                
                print("✓ Map loaded successfully")
                
                # Also open in browser as backup (can be removed if embedding works)
