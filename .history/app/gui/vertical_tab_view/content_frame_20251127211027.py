...
class TabContent(ctk.CTkFrame):
    ...
    def execute_python(self):
        ...
        python_code = self.results_section.PythonFrame.code_textbox.get(
            "1.0", "end-1c"
        ).strip()
        if not python_code:
            self.results_section.PythonFrame.clear_code()
    ...
    def compile_sql(self):
        ...
        if isinstance(compilation_result, Success):
            ...
            self.results_section.PythonFrame.set_code(python_code)
            self.error_section.clear_error()
        else:
