...
class TabContent(ctk.CTkFrame):
    ...
    def execute_python(self):
        ...
        python_code = self.results_section.python_frame.code_textbox.get(
            "1.0", "end-1c"
        ).strip()
        if not python_code:
            self.results_section.python_frame.clear_code()
            self.results_section.table_frame.clear_table()
        ...
        if isinstance(execution_result, Success):
            ...
            self.results_section.table_frame.set_table(data_frame)
            self.error_section.clear_error()
            
            # Extract and store GEE metadata if this was a GEE query
            gee_metadata = self.extract_gee_metadata(self.last_sql_query)
            self.results_section.table_frame.set_gee_metadata(gee_metadata)
        else:
    ...
    def compile_sql(self):
        ...
        if isinstance(compilation_result, Success):
            ...
            self.results_section.python_frame.set_code(python_code)
            self.error_section.clear_error()
        else:
